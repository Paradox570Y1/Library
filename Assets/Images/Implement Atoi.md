...
class Solution {
    public int myAtoi(String s) {
        ...
        if(sign_pos){
            if(ans.compareTo(BigInteger.valueOf(Integer.MAX_VALUE))==1)return Integer.MAX_VALUE;
            return ans.intValue();
        }
        //if negative
        ans = ans.negate();
        if(ans.compareTo(BigInteger.valueOf(Integer.MIN_VALUE))==-1)return Integer.MIN_VALUE;
        return ans.intValue();
    }
}
```
...
class Solution {
    public int myAtoi(String s) {
        int ans=0;
        boolean flag0 =false,flag_neg =false;
        int i=0;
        while(i<s.length() && s.charAt(i)==' ')i++;
        int temp = i;
        if(i<s.length() && i==temp){
            if(s.charAt(i)=='-'){
                flag_neg = true;
                i++;
            }
        ...
        for(;i<s.length();i++){
            ...
            if(c>='0' && c<='9'){
                if(flag_neg){
                    if(ans>=(Integer.MIN_VALUE+(c-'0'))/10)
                    ans = ans*10 - (c-'0');
