...
class Solution {
    ...
    private long helper(int pos, long sum) {
        if (pos == size) {
            return (Long.bitCount(sum) == bitCnt)? 1:0;
        }
        String s = pos + "|" + sum;
        if (memo.containsKey(s)) return memo.get(s);
        long total = 0;
        for (int i = 0; i < n; i++) {
            long new_sum = addSum(sum, i);
            long canTake = helper(pos + 1, new_sum);
            total = (total + (canTake * nums[i]) % MOD) % MOD;
        }
        memo.put(s, total);
        return total;
    }

    private long addSum(long sum, int bit) {
        while ((sum & 1L << bit) != 0) {
            sum ^= (1L << bit);
            bit++;
        }
...
Slightly better
As memo key  is long based instead of string
```java
class Solution {
    int n, size, bitCnt;
    int MOD = (int)1e9 + 7;
    ...
    private long helper(int pos, long sum) {
        ...
        long key = (pos * 1000000000000L) + sum;
