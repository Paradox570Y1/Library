...
![400](https://assets.leetcode.com/uploads/2024/01/04/example0alicebob-1.png)

**Example 1:**

![300](https://assets.leetcode.com/uploads/2024/01/04/example1alicebob.png)
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                if (points[i][1] >= points[j][1]) {
                    boolean aliceAloneWithBob = true;
                    for (int k = i+1; k < j; k++) {
                        if (points[i][1] >= points[k][1] && points[k][1] >= points[j][1]) {
                            aliceAloneWithBob = false;
                            break;
                        }
                    }
                    if(aliceAloneWithBob) {
                        count++;
                    }
                }
            }
...
class Solution {
    public int numberOfPairs(int[][] points) {
        ...
        int count = 0;
        for (int i = 0; i < n; i++) {
            int midY = Integer.MIN_VALUE;
            for (int j = i+1; j < n; j++) {
                if (points[i][1] >= points[j][1]) {
                    if (midY != Integer.MIN_VALUE && midY >= points[j][1]) {
                        continue;
                    }
                    midY = Math.max(midY, points[j][1]);
                    count++;
                }
            }
        }
